...
AddVIPRank(VIP_RANK_SILVER, "Silver", Color(192, 192, 192), 1, "Basic VIP benefits", true)
AddVIPRank(VIP_RANK_GOLD, "Gold", Color(255, 215, 0), 2, "Enhanced VIP benefits", true)
AddVIPRank(VIP_RANK_DIAMOND, "Diamond", Color(185, 242, 255), 3, "Premium VIP benefits", true)
AddVIPRank(VIP_RANK_PRISM, "Prism", Color(255, 105, 180), 4, "Exclusive special VIP (non-purchasable)", false)
  ...
  IonRP.Database:PreparedQuery(query, {}, function(data)
    if data and #data > 0 then
      ...
      IonRP.Database:PreparedQuery(
        ...
        function(err)
          print("[IonRP VIP] Error cleaning up expired VIPs: " .. err)
        end
      )
    end
  end, function(err)
