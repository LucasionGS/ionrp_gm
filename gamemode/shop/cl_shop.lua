...
function IonRP.ShopUI:Open(shopIdentifier, shopName, shopDescription, taxRate, items)
  ...
  moneyLabel.Think = function(self)
    ...
    self:SizeToContents()
    self:SetPos(frameWidth - self:GetWide() - cfg.Padding, 15)
  end
...
function IonRP.ShopUI:CreateItemPanel(parent, shop, shopItem)
  ...
  icon.Paint = function(self, w, h)
    draw.RoundedBox(6, 0, 0, w, h, Color(45, 45, 55, 150))
  end
      ...
      if hasAmount > 0 then
        ...
        if hasAmount > 1 then
          menu:AddOption("Sell Bulk...", function()
            IonRP.Dialog:RequestString("Sell Items", "How many would you like to sell? (You have " .. hasAmount .. ")", "1", function(value)
              local amount = tonumber(value)
              if amount and amount > 0 then
